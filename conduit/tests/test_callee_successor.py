...
def test_pick_modern_callee_prefers_matching_verb():
    ...
    assert (
        pick_modern_callee("openai.FineTune.list", candidates)
...
def test_pick_modern_callee_skips_legacy_aliases():
    ...
    assert pick_modern_callee("openai.Completion.create", candidates) == (
...
def test_same_path_legacy_pairs_not_self_rewrite():
    ...
    assert by_old["openai.Completion.create"] == "openai.completions.create"
