...
- `from_version` — the version the project is on now. Catalog packets with floor `0` are stamped from the source packet `installed_version` before export-delta runs.
